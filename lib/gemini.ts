...
export async function requestMusicalPlan(
  ...
  const userPrompt = `Create a rhythmically and melodically varied musical event plan for the attached video.
ABSOLUTE RULES (reject simple repeating 4-note loops like C-E-G-B cycling):
...
12. JSON ONLY.
13. STARTING DIVERSITY: Do NOT always begin on C4 or D4. First 6-8 events must span at least two distinct octaves. If mood is happy/uplifting pick a non-C tonic (e.g. D, E, G, A). If mood is tense/somber pick a minor/non-C root (e.g. A, D, E) unless visuals clearly suggest brightness.
14. PITCH DISTRIBUTION: Ensure no more than 45% of events reside in the narrow mid band (C4–B4). Use at least 3 notes <= E3 (MIDI 52) and at least 3 notes >= B5 (MIDI 83) for clips >= 8s.
15. AVOID TEMPLATE FEEL: Vary contour so two different videos with similar motion do not produce the same ascending ladder C4..D4..E4.. etc. Introduce early contrast (a leap or register drop) within first 5 events.
...
Return ONLY the JSON object.`;
  ...
  // Enhanced scale selection: explicit major for positive / uplifting; minor for negative / tense.
  let scaleType: 'minor' | 'major' | 'dorian' | 'mixolydian' = 'dorian';
  ...
  if (positiveRe.test(moodStr) && !negativeRe.test(moodStr)) scaleType = 'major';
  else if (negativeRe.test(moodStr) && !positiveRe.test(moodStr)) scaleType = 'minor';
  else if (buildRe.test(moodStr)) scaleType = 'mixolydian';
  else if (resolveRe.test(moodStr)) scaleType = 'dorian';

    // --- Seeded root & register selection to differentiate clips ---
    const hashString = (str:string) => {
      let h = 0; for (let i=0;i<str.length;i++) { h = (h*131 + str.charCodeAt(i)) >>> 0; } return h;
    };
    const seedBase = hashString(JSON.stringify(extraContext?.videoAnalysis?.segments||[]) + ':' + (extraContext?.videoAnalysis?.durationMs||'0'));
    const seedVariant = (seedBase % 9973);
    // Candidate tonic centers (MIDI) for major/minor contexts (around octave 4 but varied)
    const majorRoots = [50, 52, 55, 57, 59, 62, 64]; // D3, E3, G3, A3, B3, D4, E4
    const minorRoots = [45, 48, 50, 53, 57, 60];      // A2, C3, D3, F3, A3, C4
    const pick = (arr:number[]) => arr[seedVariant % arr.length];
  ...
  const clamp = (n:number)=>Math.min(96, Math.max(36, n));
    ...
    let segmentIntensity = (t:number) => 0.5;
  ...
  const enrichPlan = (evts: InternalEvent[], target: number): InternalEvent[] => {
    ...
    const maxEvents = 80;
    const rng = (seed => () => (seed = (seed * 16807) % 2147483647) / 2147483647)(Date.now() % 2147483647);
  ...
  try {
    ...
    // Define motif interval pools (in semitones relative movement)
    const motifPool: number[][] = [
      [0, +2, -1, +4],
      [0, +3, -2, +5],
      [0, +2, +5, -3],
      [0, +7, -5, +2],
  ...
  const rebuilt: InternalEvent[] = [];
      ...
      for (const seg of segs) {
        ...
        const chooseDuration = () => {
          if (motion >= 8) return 150 + (globalStep*37 % 160); // 150-310
          if (motion >= 6) return 240 + (globalStep*53 % 320); // 240-560
          if (motion >= 4) return 400 + (globalStep*61 % 480); // 400-880
          if (motion >= 2) return 900 + (globalStep*29 % 500); // 900-1400
          return 1200 + (globalStep*41 % 700); // 1200-1900
        };
