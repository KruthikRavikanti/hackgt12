...
function synthesizePlanToWavBase64(plan: { events: MusicalPlanEvent[] }, targetDuration?: number): string {
  ...
  for (const evt of plan.events) {
    ...
    for (let i = 0; i < durSamples; i++) {
      ...
      const env = i < 100 ? i / 100 : i > durSamples - 200 ? Math.max(0, (durSamples - i) / 200) : 1;
      const sample = Math.sin((2 * Math.PI * freq * i) / sampleRate) * vel * 0.4 * env;
      left[idx] += sample;
      right[idx] += sample * 0.9; // slight channel difference
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  musicalPlan.events = fillGapsWithMedian(musicalPlan.events, detectedScaleRefined);
      ...
      // Fallback stub sized to requested duration (NOTE: causes monotony if always hit)
      const base = targetDuration || 8000;
      const step = base / 12;
      const fallbackNotes = ['C4','E4','G4','B4','D5','A4','F4','G4','E4','C4','D4','F4'];
      const fallbackTypes: NoteType[] = ['quarter','eighth','half','quarter','eighth','whole','quarter','half','eighth','quarter','half','eighth'];
      musicalPlan = { events: Array.from({ length: 12 }).map((_, i) => ({
        noteName: fallbackNotes[i % fallbackNotes.length],
        time: Math.round(i * step),
        noteType: fallbackTypes[i % fallbackTypes.length],
        duration: NOTE_TYPE_MS[fallbackTypes[i % fallbackTypes.length]],
        velocity: 70 + (i % 5) * 5
      })) };
